...
スクレーパーで収集したデータをブラウザ上で表示するダッシュボードを起動できます。
...
スクレーパーとダッシュボードを同時に実行するには、別々のターミナルウィンドウで起動します：
...
これでスクレーパーが5分ごとにデータを収集し、ダッシュボードがそのデータをリアルタイム表示します。

### ダッシュボードのカスタマイズ

`dashboard.py` の `CONFIG` セクションで設定を変更できます：

```python
CONFIG = {
    "csv_file": "tdr_analysis_log.csv",  # CSVファイル名
